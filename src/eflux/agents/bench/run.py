...
def run_episode(make_agent, *, n_ticks: int, tick_h: float) -> tuple[Simulator, object]:
...
def score(name: str, make_agent, *, n_ticks: int, tick_h: float) -> EpisodeMetrics:
    """Run one candidate through an episode and measure it (shared by the benchmark
    and the PPO eval so they score identically)."""
    sim, vpp = run_episode(make_agent, n_ticks=n_ticks, tick_h=tick_h)
    return _measure(name, sim, vpp, n_ticks)


def run_benchmark(*, n_ticks: int = 144, tick_minutes: float = 10.0) -> list[EpisodeMetrics]:
    """Score every candidate on the fixed scenario. Default: a full sim-day at 10-min
    ticks (144 ticks)."""
    tick_h = tick_minutes / 60.0
    return [score(name, make, n_ticks=n_ticks, tick_h=tick_h) for name, make in candidates().items()]
...
def main() -> None:
    ...
    args = ap.parse_args()
    rows = run_benchmark(n_ticks=args.ticks, tick_minutes=args.tick_minutes)
