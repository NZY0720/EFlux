...
export default function CompetitionDetail() {
  ...
  return <div className="mx-auto w-full max-w-[1400px] space-y-6 px-4 py-5 md:p-6">
    ...
    <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h1 className="text-xl font-semibold text-[var(--text)]">{competition.title}</h1><StatusPill tone={statusTone(competition.status)}>{competition.status}</StatusPill></div><p className="mt-2 max-w-3xl text-sm text-[var(--text-muted)]">{competition.description}</p></div>{token && competition.status === "open" && <Link to={`/competitions/${competition.slug}/submit`} className="eflux-btn eflux-btn-primary h-9 px-4 text-sm">Submit to Season 0</Link>}</div>
