  ...
  return <div className="mx-auto w-full max-w-4xl space-y-6 px-4 py-5 md:p-6">
    <DashboardCard>
      ...
      <form onSubmit={deploy} className="space-y-4">
        {step === 1 && <div className="grid gap-2 sm:grid-cols-2"><input aria-label="Agent name" required value={name} onChange={(e) => setName(e.target.value)} placeholder="agent name" className="eflux-input rounded-md px-3 py-2 text-sm outline-none sm:col-span-2" />{Object.keys(presets).map((choice) => <button key={choice} type="button" aria-pressed={preset === choice} onClick={() => choosePreset(choice as PresetName)} className={`rounded-md border p-3 text-left text-sm ${preset === choice ? "border-[var(--accent)] bg-[var(--accent-soft)]" : "border-[var(--border)] hover:bg-[var(--surface-hover)]"}`}><span className="font-semibold">{choice}</span><span className="mt-1 block text-xs text-[var(--text-muted)]">{choice === "Custom" ? "Keep every setting editable." : `PV ${presets[choice]!.pv}kW · Battery ${presets[choice]!.batt}kWh`}</span></button>)}</div>}
