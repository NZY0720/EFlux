      ...
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="hidden items-center gap-1 lg:flex">
          ...
          <div className="relative" ref={exploreRef}>
            ...
            {exploreOpen && <div id="explore-menu" role="menu" aria-label="Explore" className="absolute right-0 top-full mt-2 w-48 rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)] p-1.5 shadow-lg">{EXPLORE_ITEMS.map((item) => <div role="none" key={item.to}>{navLink(item)}</div>)}</div>}
          </div>
        </div>
      </div>
