...
async def _headers(client) -> dict[str, str]:
    magic = await client.post("/auth/magic-link", json={"email": "v1-agent@example.com"})
...
@pytest.mark.asyncio
async def test_external_protocol_v1_requires_product_and_physical_purpose(client):
...
@pytest.mark.asyncio
async def test_protocol_v1_batch_echoes_client_reference(client):
    headers = await _headers(client)
