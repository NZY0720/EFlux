...
def test_v1_observation_is_signed_finite_and_carries_delivery_runtime_state():
    obs = encode_obs(_context(), _valuation(), obs_version=OBS_V1)
...
def test_new_policy_and_env_defaults_use_v1_width():
    assert BCNet().net[0].in_features == OBS_DIM_V1
    env = VPPPrimitiveEnv({"seed": 1, "episode_ticks": 1})
    obs, _ = env.reset(seed=1)
    assert env.obs_version == OBS_V1
    assert obs.shape == (OBS_DIM_V1,)
