...
def _balance_orders(decision):
    return [order for order in decision.orders if order.purpose.value == "balance"]
...
def test_pure_pv_export_quotes_at_floor():
    ...
    decision = agent.decide(_make_ctx(pv_kw=5.0, load_kw=1.0, markup_floor=0.1))
    orders = _balance_orders(decision)
    assert len(orders) == 1
    assert orders[0].side == "sell"
    # Floor = markup_floor * price_ref = 0.1 * 50 = 5.0
    assert orders[0].price == Decimal("5.0000")
    assert orders[0].qty_kwh == Decimal("4.0000")


def test_buy_price_equals_price_ref_when_deficit():
    """Load > PV → buy at price_ref (willing to pay retail to cover load)."""
    agent = TruthfulAgent(price_ref=Decimal("50.0"))
    decision = agent.decide(_make_ctx(pv_kw=0.5, load_kw=3.0))
    orders = _balance_orders(decision)
    assert len(orders) == 1
    assert orders[0].side == "buy"
    assert orders[0].price == Decimal("50.0000")
    assert orders[0].qty_kwh == Decimal("2.5000")


def test_balanced_position_no_order():
    agent = TruthfulAgent()
    assert agent.decide(_make_ctx(pv_kw=2.0, load_kw=2.0, soc_kwh=3.5)).is_empty
...
def test_zero_renewable_floor_is_valid_in_negative_price_market():
    agent = TruthfulAgent(price_ref=Decimal("50.0"))
    decision = agent.decide(_make_ctx(pv_kw=5.0, load_kw=1.0, markup_floor=0.0))
    orders = _balance_orders(decision)
    assert len(orders) == 1
    assert orders[0].price == Decimal("0.0000")
...
def test_demand_beta_raises_bid_with_deficit():
    ...
    assert len(orders) == 1
    assert orders[0].side == "buy"
    assert orders[0].price == Decimal("56.2500")
...
def test_demand_beta_bid_capped_at_price_cap_mult():
    ...
    assert decision.orders[0].price == Decimal("75.0000")


def test_demand_beta_defaults_to_legacy_flat_bid():
    agent = TruthfulAgent(price_ref=Decimal("50.0"))
    decision = agent.decide(_make_ctx(pv_kw=0.5, load_kw=3.0))
    assert decision.orders[0].price == Decimal("50.0000")
...
def test_demand_beta_sees_resting_book_deficit():
    ...
    assert orders[0].price == Decimal("75.0000")
...
def test_battery_band_sells_stored_energy_at_night():
    ...
    battery_orders = []
    for _ in range(agent.battery_quote_every_n_decisions + 1):
        battery_orders = [
            order for order in agent.decide(ctx).orders if order.purpose.value == "battery"
        ]
        if battery_orders:
            break
    assert battery_orders, "expected a battery sell quote within the cooldown window"
    sell = battery_orders[0]
    assert sell.side == "sell"
    # delivery cost = 50 / sqrt(0.9) ≈ 52.7
    assert Decimal("52") < sell.price < Decimal("53")
    assert sell.qty_kwh >= agent.min_qty
...
def test_battery_band_buys_back_when_depleted():
    ...
    for _ in range(agent.battery_quote_every_n_decisions + 1):
        ...
        if battery_orders:
            break
    assert battery_orders, "expected a battery recharge bid"
    buy = battery_orders[0]
    assert buy.side == "buy"
