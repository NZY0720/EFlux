...
from eflux.market.gateway import GatewayRiskLimits, TradingGatewayV1
...
def test_replace_reuses_credit_reserved_by_original_order():
    gateway = TradingGatewayV1(limits=GatewayRiskLimits(credit_limit_usd=Decimal("0.05")))
    gateway.register_participant(
        participant_id=1,
        params=VPPParams(
            pv_kw_peak=0,
